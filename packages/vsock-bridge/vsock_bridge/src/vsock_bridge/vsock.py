...
class VsockConnectionWorker(threading.Thread):
    def __init__(self, conn: socket.socket, addr: Tuple[int, int], handler: VsockConnectionHandler, name: str | None = None):
...
class VsockServer(threading.Thread):
    def __init__(self, cid: int, port: int, max_connections: int, name: str | None = None):
    ...
    def run(self) -> None:
        while not self._stop.is_set():
            ...
            if not handler:
                ...
                continue
            w = VsockConnectionWorker(conn, addr, handler, name=f"VsockWorker:{peer_cid}:{peer_port}")
