...
class Config:
    ...
    def match(self, usb_info, usb_rule):
        ...
        logger.debug(
            "Checking device class %s, subclass %s, protocol %s",
    ...
    def vm_for_usb_device(self, usb_info):
        try:
            ...
            for usb_rule in self.config.get("usbPassthrough", []):
                ...
                if found:
                    ...
                    if target_vm:
                        ...
                        logger.info(
                            "Found allowed VMs %s for %s", allowed_vms, usb_dev_name
                        )
                    else:
                        logger.error(
            ...
            logger.error(
                "Failed to find VM for USB device in the configuration file: %s", e
            )
        return None
